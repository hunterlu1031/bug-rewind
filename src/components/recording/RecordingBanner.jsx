...
export function RecordingBanner() {
  const { temporarySteps, stopRecording, screenshotCount } = useRecordingContext();
      ...
      <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-2">
        <div className="flex items-center gap-2 text-sm">
          ...
          <span className="text-red-700/80">
            — {temporarySteps.length} step{temporarySteps.length !== 1 ? 's' : ''}
            · {screenshotCount}/{MAX_SCREENSHOTS_PER_RECORDING} screenshots
          </span>
        </div>
        <button
          type="button"
