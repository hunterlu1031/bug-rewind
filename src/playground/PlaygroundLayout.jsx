...
function PlaygroundChrome() {
  ...
  return (
    <div className="playground-shell min-h-full">
      <header className="sticky top-0 z-20 border-b border-pg-line/60 bg-pg-surface/90 shadow-pg backdrop-blur-md">
      ...
      <div
        className={`mx-auto w-full max-w-[1400px] px-4 sm:px-8 lg:px-12 ${
          isReplaying ? 'space-y-2 py-3' : 'space-y-5 py-6'
        }`}
      >
        <PlaygroundInstructions forceCollapsed={isReplaying} />
      ...
      <div className="mx-auto w-full max-w-[1400px] px-4 pb-12 sm:px-8 lg:px-12">
        <div
          ...
          className="relative min-h-[640px] overflow-hidden rounded-3xl border border-pg-glow/50 bg-pg-surface shadow-pg-lg ring-1 ring-pg-line/50 sm:min-h-[720px]"
        >
          <div
            className={
              isReplaying ? 'pointer-events-none select-none' : undefined
            }
            aria-hidden={isReplaying || undefined}
            inert={isReplaying ? true : undefined}
          >
            <PlaygroundStoreNav />
          ...
          {isReplaying && (
            <div
              className="absolute inset-0 z-30 cursor-not-allowed bg-pg-ink/[0.04]"
              data-testid="playground-replay-shield"
              role="presentation"
              aria-hidden
            />
          )}
